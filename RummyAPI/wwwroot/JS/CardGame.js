...
//define Card Ranks
function Rank(face, value, weight){
	this.face = face;
	this.value = value;
	this.weight = weight;
}
...
function Card(face){
	...
	this.weight = rank.weight; //10 for Jack
...
Node.prototype.computeWeight = function(){
...
function Pool(deckCount, playerNames, cutOffWeight, dropWeight, middleDropWeight){
    ...
    this.middleDropWeight = middleDropWeight;
    return this;  
}
...
Pool.prototype.startNextGame = function(){
	...
	//TODO need to build cards here
	
	//reset player statuses
	for(var i = 0; i < this.players.length; ++i){
		if(!this.players[i].isOut){ //ABID added this keyword for players array 
            this.players[i].isDropped = false;
...
function Game(pool, cards, players, openerIndex){
    ...
    this.discards = []; //ABID because discards in cutDeck is undefined
	shuffle(this.cards);
}
...
Game.prototype.cutDeck = function(index){
	...
	this.crds = this.cards.concat(topCards);
	...
	//this.discards.push(this.cards.shift()); //ABID What are we doing with this? This i
...
Game.prototype.updateWeights = function(){
	var self = this;
	...
	for(var i = 0; i < this.players.length; ++i){
		...
		if(player.isOut){
			if(player.isDropped){
				weight = this.pool.dropWeight;
			}
			else if(player.isMiddleDropped){
				weight = this.pool.middleDropWeight;
			}
			else{
				var bestOption = players[i].findBestOption();
				weight = bestOption.weight;
			}
			this.gameWeights[i] = weight;
			player.weight += weight;
			
			if(player.weight > this.pool.cutOffWeight){
				player.isOut = true;
			}
		}
	}
...
//function to determine the best card arrangement option which results in least weight
Player.prototype.findBestOption = function(){
	...
	//let us find the option with the least weight
	var bestNode = rootNode;
	bestNode.computeWeight();
	
	nodes.forEach(function(node){
		var weight = node.computeWeight();
		if(weight < bestNode.weight){
			bestNode = node;
		}
	});
        ...
        sequenceSets.forEach(function(set){
    		...
    		otherSets.forEach(function(set){
        		...
        		remainingSets.forEach(function(set){
            		...
            		if(currentNode.computeWeight() == 0){
